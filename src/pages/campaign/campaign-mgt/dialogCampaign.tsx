...
const DialogCampaign = (props: DialogInfoProps) => {
  ...
  function handleKeyword (action: string, index? : number) {
    ...
    if(action === 'remove') {
      if(index) {
        ...
        console.log('remove', index, values)
        values.splice(index, 1);
        console.log("remove value", values);
        setKeyword(values)
      }
    }
          ...
          <div id={`campaign-master`}>
            <Grid container spacing={6} style={{paddingLeft: '1.5rem'}}>
              <Grid item sm={12} xs={12}>
                <TextField fullWidth label='Campaign Name' value={campaignName}
                ...
                <Grid container sx={{ mt: 4.75 }}>
                  <Grid item xs={12} sx={{ px: 0 }}>
                    <Button
                      ...
                      onClick={() =>{ setCount(count + 1); handleKeyword('add') }}
                    >
                      Add Keyword
                    </Button>
                  </Grid>
