...
const ChannelBySentimentComparison = (props: LineProps) => {
  ...
  const { resultChannelBySentiment, loadingChannelBySentiment } = GetChannelBySentiment(
    params?.campaign,
    params?.date,
    params?.endDate,
    params?.period,
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultChannelBySentiment?.value
  ...
  useEffect(() => {
    if (resultChannelBySentiment) {
      const labels = chartLabel(resultChannelBySentiment)
      setLabel(labels)
    }
  }, [t])
  ...
  useEffect(() => {
    if (resultChannelBySentiment) {
      const dailyMessageData = resultChannelBySentiment
      if (dailyMessageData) {
  ...
  return (
    <Paper sx={{ border: `3px solid #fff`, borderRadius: 1 }} >
      {loadingChannelBySentiment && <LinearProgress style={{ width: '100%' }} />}
