...
const ChannelByBullyLevel = (props: LineProps) => {
  ...
  const { resultChannelByBullyLevel, loadingChannelByBullyLevel } = GetChannelByBullyLevel(
    params?.campaign,
    params?.date,
    params?.endDate,
    params?.period,
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultChannelByBullyLevel?.value
  ...
  useEffect(() => {
    if (resultChannelByBullyLevel) {
      const dailyMessageData = resultChannelByBullyLevel
      if (dailyMessageData) {
  ...
  return (
    <Paper sx={{ border: `3px solid #fff`, borderRadius: 1 }} >
      {loadingChannelByBullyLevel && <LinearProgress style={{ width: '100%' }} />}
