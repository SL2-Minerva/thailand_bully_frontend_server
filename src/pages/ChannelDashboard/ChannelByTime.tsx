...
const ChannelByTime = (props: LineProps) => {
  ...
  const { resultChannelByTime, loadingChannelByDay } = GetChannelByTime(params?.campaign, params?.date, params?.endDate, params?.period, params?.keywordIds);
  ...
  useEffect(() =>{
    if(resultChannelByTime) {
      const labels = chartLabel(resultChannelByTime);
      setLabel(labels);
    }
  },[t])
    ...
    useEffect(() => {
        if(resultChannelByTime) {
        const dailyMessageData = resultChannelByTime;
        if(dailyMessageData) {
