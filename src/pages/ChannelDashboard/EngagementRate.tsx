...
import { GetEngagementRate, GetEngagementRatePrevious } from 'src/services/api/dashboards/channel/ChannelDashboardApi'
...
const EngagementRate = (props: LineProps) => {
  ...
  const { resultEngagementRate, loadingEngagementRate } = GetEngagementRate(params?.campaign, params?.date, params?.endDate, params?.period, params?.keywordIds);
  const { resultEngagementRatePrevious, loadingEngagementRatePrevious } = GetEngagementRatePrevious(params?.campaign, params?.date, params?.endDate, params?.period, params?.keywordIds);
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultEngagementRatePrevious?.value?.previous_period;
  ...
  const options = {
    ...
    useEffect(() => {
        ...
        const currentEngagementData = resultEngagementRate;
        const previousEngagementData = resultEngagementRatePrevious;
    ...
    return (
      <Card>
        {loadingEngagementRate && loadingEngagementRatePrevious && (
        <LinearProgress
            style={{ width: "100%" }}
        />
        )}
