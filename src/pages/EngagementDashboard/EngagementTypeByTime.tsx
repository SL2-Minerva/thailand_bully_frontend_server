...
const onCapture = () => {
  const pictureId = document.getElementById('typeByTime')
  if (pictureId) {
    htmlToImage.toPng(pictureId, { backgroundColor: '#fff' }).then(function (dataUrl) {
      saveAs(dataUrl, 'Daily Engagement Type By Time.png')
    })
  }
}
...
const EngagementTypeByTime = (props: LineProps) => {
  ...
  const chartDatasets = (data: any) => {
    ...
    for (let i = 0; i < total?.length; i++) {
      ...
      keywordName = total[i]?.keyword_name || total[i]?.name || ''
    ...
    <Paper sx={{ border: `3px solid #fff`, borderRadius: 1 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <span style={{ display: 'flex', justifyContent: 'flex-start' }}>
          <CardHeader
            title={<Translations text='Daily Engagement Type By Time' />}
            ...
            title={
              <span>
                <Typography variant='h6' sx={{ color: 'white' }}>
                  <Translations text='engagementChart12Title' />
                </Typography>
                <Typography variant='body2' sx={{ color: 'white' }}>
                  <Translations text='engagementChart12Description' />
                </Typography>
              </span>
            }
          >
