...
import { GetMessagesByAccount } from 'src/services/api/dashboards/voice/VoiceDashboardAPIs'
...
const MessagesByAccount = (props: LineProps) => {
  ...
  const { resultMessagesByAccount, loadingMessagesByAccount } = GetMessagesByAccount(
    params?.campaign,
    params?.date,
    params?.endDate,
    params?.period,
  ...
  const chartDatasets = (data: any) => {
    ...
    const color = loadingMessagesByAccount && keywordsColor ? keywordsColor  : GraphicColors
  ...
  useEffect(() => {
    if (resultMessagesByAccount) {
      const labels = chartLabel(resultMessagesByAccount)
      setLabel(labels)
    }
  }, [t])
  ...
  const reportNo = '2.2.006'

  const chartTitle = chartId + ', Report Level 2(' + reportNo + ')'

  return (
    <Paper sx={{ border: `3px solid #fff`, borderRadius: 1 }} square variant='outlined'>
      {loadingMessagesByAccount && <LinearProgress style={{ width: '100%' }} />}
...
export default MessagesByAccount
