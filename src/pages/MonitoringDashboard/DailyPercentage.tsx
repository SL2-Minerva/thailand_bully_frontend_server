...
interface MessageData {
  ...
  highlight: boolean
  chartId: string
  resultDailyMonitoring: any
  loadindDailyMonitoring: boolean
  keywordGraphColors: any
}

const DailyPercentage = (props: MessageData) => {
    const { resultDailyMonitoring, loadindDailyMonitoring, keywordGraphColors } = props
    ...
    const initValue = {
      ...
      datasets: [
        {
          data: [],
          backgroundColor: keywordGraphColors,
          hoverOffset: 4
        }
      ]
    }
    ...
    useEffect(() => {
      if (resultDailyMonitoring) {
        const currentMessageData = resultDailyMonitoring?.prcentage_of_messages_current
        const previousMessageData = resultDailyMonitoring?.prcentage_of_messages_previous
  
        if (currentMessageData) {
          const currentDataset = chartDataset(currentMessageData, 'current', keywordGraphColors)
        ...
        if (previousMessageData) {
          const previousDataset = chartDataset(previousMessageData, 'previous', keywordGraphColors)
    ...
    }, [resultDailyMonitoring, keywordGraphColors])
  
    return (
      <Paper sx={{ border: `3px solid #fff`, borderRadius: 1, minHeight: 620 }} >
        {loadindDailyMonitoring && <LinearProgress style={{ width: '100%' }} />}
            ...
            <Grid item xs={12}>
              <Box pl={{ xs: 1.3 }} pr={{ xs: 1 }} sx={{ display: 'flex', justifyContent: 'center' }}>
                <CustomeLabels
                  ...
                  labels={getLabelColor(keywordGraphColors)}
                  color={getColors(keywordGraphColors)}
