...
import { GetBullyTypeByAccount } from 'src/services/api/dashboards/bully/BullyDashboardAPI'
import MessageDetail from '../ChannelDashboard/MessageDetail'
  
const BullyTypeByAccount = (props: LineProps) => {
  ...
  const {resultBullyTypeByAccount, loadingBullyTypeByAccount} =  GetBullyTypeByAccount(params?.campaign, params?.date, params?.endDate, params?.period, params?.keywordIds);
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultBullyTypeByAccount?.value;
  ...
  const chartDatasets = (data:any) => {
    ...
    useEffect(() => {
        if(resultBullyTypeByAccount) {
        const dailyMessageData = resultBullyTypeByAccount;
        if(dailyMessageData) {
    ...
    const reportNo = '6.2.016';

    const chartTitle = chartId + ", Report Level 2(" + reportNo + ")";

    return (
      <Card>
         {loadingBullyTypeByAccount && (
            <LinearProgress
                style={{ width: "100%" }} 
            />
            )} 
        <span style={{ display: 'flex', justifyContent: 'flex-start' }}>
          <CardHeader
            title="Bully Type: Daily Message by Account"
...
export default BullyTypeByAccount