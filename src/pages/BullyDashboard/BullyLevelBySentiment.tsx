...
import { GetBullyBySentiment } from 'src/services/api/dashboards/bully/BullyDashboardAPI'
...
const BullyLevelBySentiment = (props: LineProps) => {
  ...
  const { resultBullyBySentiment } = GetBullyBySentiment(params?.campaign, params?.date, params?.endDate, params?.period);
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultBullyBySentiment?.daily_message;
  ...
  const chartDatasets = (data:any) => {
    ...
    useEffect(() => {
        if(resultBullyBySentiment) {
        const dailyMessageData = resultBullyBySentiment;
        if(dailyMessageData) {
