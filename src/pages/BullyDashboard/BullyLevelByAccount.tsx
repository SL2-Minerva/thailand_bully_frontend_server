...
import { GetBullyByAccount } from 'src/services/api/dashboards/bully/BullyDashboardAPI'
import MessageDetail from '../ChannelDashboard/MessageDetail'
  
const BullyLevelByAccount = (props: LineProps) => {
  ...
  const { resultBullyByAccount, loadingBullyByAccount } = GetBullyByAccount(params?.campaign, params?.date, params?.endDate, params?.period, params?.keywordIds);
  ...
  const getKeywordId = (dataset: InteractionItem[]) => {
    ...
    const dailyMessageData = resultBullyByAccount?.value;
  ...
  const chartDatasets = (data:any) => {
    ...
    useEffect(() => {
        if(resultBullyByAccount) {
        const dailyMessageData = resultBullyByAccount;
        if(dailyMessageData) {
    ...
    const reportNo = '6.2.006';

  const chartTitle = chartId + ", Report Level 2(" + reportNo + ")";

    return (
      <Card>
         {loadingBullyByAccount && (
            <LinearProgress
                style={{ width: "100%" }} 
            />
            )} 
        <span style={{ display: 'flex', justifyContent: 'flex-start' }}>
          <CardHeader
            title="Bully Level: Daily Messages by Account"
...
export default BullyLevelByAccount